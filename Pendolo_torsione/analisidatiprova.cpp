...
double contadatipos(vector<double> p, vector<double>& ap, vector<double>& at, vector<double> t,double temp);
...
ofstream output("fitmassimi.txt");
    ...
    // cout<<soglia<<endl;
    double soglia;
    vector<double> datintermedi;
    vector<double> tempintermedi;
    double temp=0;

    while(temp<p.size()&&(temp+1 < p.size())){
            soglia=p.at((contadatipos(p,datintermedi,tempintermedi,t,temp)/2)-1);
            cout<<"La soglia in questo particolare intervallo: "<<soglia<<endl;
            for(int i=0;i<datintermedi.size();i++){
                    if(datintermedi.at(i)>soglia){
                        datifity.push_back(datintermedi.at(i));
                        datifitx.push_back(tempintermedi.at(i));
                    }
            }
            if (datifitx.size() >= 3) {
        ...
        temp+=contadatipos(p,datintermedi,tempintermedi,t,temp)+1;
        datifitx.clear();
        datifity.clear();
        datintermedi.clear();
        tempintermedi.clear();
